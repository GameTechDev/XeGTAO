...
float4 RMSampleTexture2D( const in GeometryInteraction geometrySurface, uint bindlessIndex, SamplerState sampler, uniform const int UVIndex )
{
    float2 coords = ( UVIndex == 0 )?( geometrySurface.Texcoord01.xy ):( geometrySurface.Texcoord01.zw );

#ifdef VA_RAYTRACING
    // TODO: read https://www.researchgate.net/publication/354065178_Sampling_Textures_with_Missing_Derivatives
    ...
    return g_bindlessTex2D[bindlessIndex].SampleLevel( sampler, coords, texBaseLOD + geometrySurface.BaseLODTex0 + g_globals.GlobalMIPOffset );
...
RenderMaterialInputs LoadRenderMaterialInputs( const in GeometryInteraction geometrySurface, const in ShaderMaterialConstants materialConstants )
{
...
//     BaseColorTexture = float4( 0, 1, 0, 1 ); //g_RMTexture00.SampleBias( g_samplerPointWrap, geometrySurface.Texcoord01.xy, g_globals.GlobalMIPOffset ); 
// 
//     // Triplanar projection
//     float3 blendWeights = abs(normalize(geometrySurface.ObjectPosition));
//     float3 uvw = geometrySurface.ObjectPosition * 0.5f + 0.5f;
