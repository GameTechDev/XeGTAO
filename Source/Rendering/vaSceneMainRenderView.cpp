...
{
    vaSceneRenderViewBase::ProcessInstanceBatchCommon( scene.Registry(), items, itemCount,  &m_selectionOpaque, &m_selectionTransparent, vaRenderInstanceList::FilterSettings::FrustumCull( *m_camera ), m_selectionFilter, baseInstanceIndex );
}
...
void vaSceneMainRenderView::RenderTickInternal( vaRenderDeviceContext & renderContext, vaDrawResultFlags & currentDrawResults, vaDrawAttributes::GlobalSettings & globalSettings, bool skipCameraLuminanceUpdate, DepthPrepassType depthPrepass )
{
    ...
    if( depthPrepass != DepthPrepassType::UseExisting )
    {
        ...
        if( m_workingViewspaceDepth != nullptr )
            m_workingViewspaceDepth->ClearRTV( renderContext, {0,0,0,0} );
    }
    ...
    if( depthPrepass == DepthPrepassType::DrawAndUse )
    {
        ...
        drawResults |= sceneRenderer->DrawDepthOnly( renderContext, vaRenderOutputs::FromRTDepth( {m_workingNormals, m_workingMotionVectors, m_workingViewspaceDepth}, m_workingDepth ), m_selectionOpaque, m_sortDepthPrepass, *m_camera, globalSettings );
    ...
    pp.GenerateMotionVectors( renderContext, vaDrawAttributes( *m_camera, vaDrawAttributes::RenderFlags::None, nullptr, nullptr, globalSettings ), m_workingDepth, m_workingMotionVectors, m_workingViewspaceDepth );
    ...
    bool debugViewActive = false;
    {
        ...
        if( m_settings.RenderPath == vaRenderType::PathTracing && m_pathTracer != nullptr && m_pathTracer->DebugViz() != PathTracerDebugViewType::None )
    ...
    if( m_TAA != nullptr )
    {
        ...
        drawResults |= m_TAA->Apply( renderContext, *m_camera, m_workingMotionVectors, m_workingViewspaceDepth, m_workingPreTonemapColor, m_reprojectionMatrix, m_cameraJitterDelta );
    }
...
void vaSceneMainRenderView::RenderTick( float deltaTime, vaRenderDeviceContext & renderContext, vaDrawResultFlags & currentDrawResults )
{
    ...
    globalSettings.ReprojectionMatrix  = m_reprojectionMatrix;
    globalSettings.CameraJitterDelta   = m_cameraJitterDelta;
    ...
    if( m_workingDepth == nullptr || m_workingDepth->GetWidth( ) != workingViewport.Width || m_workingDepth->GetHeight( ) != workingViewport.Height || m_workingDepth->GetSampleCount() != msaaSampleCount )
    {
        ...
        m_workingMotionVectors  = vaTexture::Create2D( GetRenderDevice(), vaResourceFormat::R16G16B16A16_FLOAT, workingViewport.Width, workingViewport.Height, 1, 1, 1, vaResourceBindSupportFlags::ShaderResource | vaResourceBindSupportFlags::UnorderedAccess | vaResourceBindSupportFlags::RenderTarget );
        m_workingMotionVectors->SetName( "WorkingMotionVectors" );

        m_workingViewspaceDepth = vaTexture::Create2D( GetRenderDevice(), vaResourceFormat::R32_FLOAT, workingViewport.Width, workingViewport.Height, 1, 1, 1, vaResourceBindSupportFlags::ShaderResource | vaResourceBindSupportFlags::UnorderedAccess | vaResourceBindSupportFlags::RenderTarget );
        m_workingViewspaceDepth->SetName( "WorkingViewspaceDepth" );
    }
