...
public:
  ...
  HRESULT Initialize( const std::wstring & path = L"" ) {
    #ifdef _WIN32
    return InitializeInternal( (path + L"dxcompiler.dll").c_str(), "DxcCreateInstance");
    #elif __APPLE__
    return InitializeInternal( (path + L"libdxcompiler.dylib").c_str(), "DxcCreateInstance");
    #else
    return InitializeInternal( (path + L"libdxcompiler.so".c_str(), "DxcCreateInstance");
    #endif
  }
