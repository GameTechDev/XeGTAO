...
class Executor {
  ...
  public:
    ...
    */
    explicit Executor(size_t N = std::thread::hardware_concurrency(), std::function<void(size_t id)> externWorkerInit = nullptr);
  ...
  private:
    ...
    void _spawn(size_t, std::function<void(size_t id)> externWorkerInit);
...
// Constructor
inline Executor::Executor(size_t N, std::function<void(size_t id)> externWorkerInit) : 
  _workers    {N},
  _notifier   {N} {
...
// Procedure: _spawn
inline void Executor::_spawn(size_t N, std::function<void(size_t id)> externWorkerInit) {
  for(size_t id=0; id<N; ++id) {
    ...
    _threads.emplace_back([this, externWorkerInit] (Worker& w) -> void {

      //_per_thread.worker = &w;
      this_worker().worker = &w;

      if( externWorkerInit )
        externWorkerInit( w._id );
