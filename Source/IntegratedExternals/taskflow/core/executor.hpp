...
class Executor {
  ...
  private:
    ...
    void _spawn(size_t, Domain, std::function<void(size_t id)> externWorkerInit);
...
inline Executor::Executor(size_t N, std::function<void(size_t id)> externWorkerInit) : 
  ...
  _spawn(N, HOST, externWorkerInit);
...
// Procedure: _spawn
inline void Executor::_spawn(size_t N, Domain d, std::function<void(size_t id)> externWorkerInit) {
