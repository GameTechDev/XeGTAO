...
struct ImGuiViewportDataDx12
{
    ...
    ~ImGuiViewportDataDx12()
    {
        ...
        for (UINT i = 0; i < g_numFramesInFlight; ++i)
        {
            SafeRelease( FrameCtx[i].RenderTarget );
            SafeRelease( FrameCtx[i].CommandAllocator );
            SafeRelease( FrameRenderBuffers[i].IndexBuffer );
            SafeRelease( FrameRenderBuffers[i].VertexBuffer );
